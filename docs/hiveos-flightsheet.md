...
- Öffentliche **HTTPS-URL** zur Datei `vds-miner-1.1.4.tar.gz` (HiveOS lädt das Paket selbst herunter)
...
| Installation URL | `https://github.com/Crypto-EU/vds-miner/releases/download/v1.1.4/vds-miner-1.1.4.tar.gz` |
...
4. **Miner** / **Logs** öffnen. Erwartete Zeilen:
   ...
   - `Worker 0  0.220 MH/s  shares A/R 0/0` (Hashrate in Megahash)
...
```
https://github.com/Crypto-EU/vds-miner/releases/download/v1.1.4/vds-miner-1.1.4.tar.gz
```
...
```
https://raw.githubusercontent.com/Crypto-EU/vds-miner/main/dist/vds-miner-1.1.4.tar.gz
```
...
`dist/vds-miner-1.1.4.tar.gz` auf beliebigen HTTPS-Host legen (Hetzner, S3, eigener Webserver). Die URL muss ohne Login funktionieren.
...
./scripts/build.sh
chmod +x hiveos/*.sh
./hiveos/package-hiveos.sh 1.1.4
# → dist/vds-miner-1.1.4.tar.gz
```
...
cd /tmp
tar -tzf vds-miner-1.1.4.tar.gz   # muss Ordner vds-miner/ zeigen
tar -C /hive/miners/custom -xzf vds-miner-1.1.4.tar.gz
...
```bash
/hive/miners/custom/custom-get https://DEINE-URL/vds-miner-1.1.4.tar.gz -f
miner start
```
...
| Symptom | Check |
|---|---|
| `Compile failed` / Binary passt nicht zu glibc | Paket **v1.1.4** (glibc 2.17, kein Kompilieren auf dem Rig). Alte 1.1.2/1.1.3-Pakete hatten eine zu neue Binary und ein kaputtes `compile.sh`. |
| Rig startet neu, sobald **LA > 4** | HiveOS-Watchdog. Miner **v1.1.4** senkt LA (Scrypt nicht mehr auf jedem GPU-Thread). Trotzdem in HiveOS: Worker → **Watchdog** → Load Average auf **30** oder aus. SSH: in `/hive/bin/wd` steht `WD_DEF_LA=$(( CORES + 20 ))` — nicht auf 4 lassen. |
...
| HiveOS zeigt 0.00 MH/s / 0 H/s | Paket **v1.1.4** (Hashrate in MH/s). API-Port **4068** nicht von einem anderen Miner belegen. |
...
- [ ] Installation URL zeigt auf `vds-miner-1.1.4.tar.gz`
