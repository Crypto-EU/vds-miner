...
- Öffentliche **HTTPS-URL** zur Datei `vds-miner-1.1.5.tar.gz` (HiveOS lädt das Paket selbst herunter)
...
| Pass | `x` |
| Extra config arguments | leer = **alle AMD-GPUs**, zwei Pipelines. Optional: `-d 0` / `-d 0,1` oder `--intensity 1` |
...
```
https://raw.githubusercontent.com/Crypto-EU/vds-miner/main/dist/vds-miner-1.1.5.tar.gz
```
...
`dist/vds-miner-1.1.5.tar.gz` auf beliebigen HTTPS-Host legen (Hetzner, S3, eigener Webserver). Die URL muss ohne Login funktionieren.
...
cd /tmp
tar -tzf vds-miner-1.1.5.tar.gz   # muss Ordner vds-miner/ zeigen
tar -C /hive/miners/custom -xzf vds-miner-1.1.5.tar.gz
...
```bash
/hive/miners/custom/custom-get https://DEINE-URL/vds-miner-1.1.5.tar.gz -f
miner start
```
...
| Symptom | Check |
|---|---|
| `Compile failed` / Binary passt nicht zu glibc | Paket **v1.1.5** (glibc 2.17, kein Kompilieren auf dem Rig). Alte 1.1.2/1.1.3-Pakete hatten eine zu neue Binary und ein kaputtes `compile.sh`. |
| Rig startet neu, sobald **LA > 4** | HiveOS-Watchdog. Miner **v1.1.5** senkt LA (Scrypt nicht mehr auf jedem GPU-Thread). Trotzdem in HiveOS: Worker → **Watchdog** → Load Average auf **30** oder aus. SSH: in `/hive/bin/wd` steht `WD_DEF_LA=$(( CORES + 20 ))` — nicht auf 4 lassen. |
...
| HiveOS zeigt 0.00 MH/s / 0 H/s | Paket **v1.1.5** (Hashrate in MH/s). API-Port **4068** nicht von einem anderen Miner belegen. |
...
- [ ] Installation URL zeigt auf `vds-miner-1.1.5.tar.gz`
