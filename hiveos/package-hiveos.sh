...
install -m 0755 "$root/hiveos/compile.sh" "$pkg/compile.sh"
...
# Sources + OpenCL headers so HiveOS can compile on the rig if the binary is glibc-incompatible
mkdir -p "$pkg/src"
cp -a "$root/src/." "$pkg/src/"
if [[ -d "$root/third_party/OpenCL" ]]; then
  mkdir -p "$pkg/third_party"
  cp -a "$root/third_party/OpenCL" "$pkg/third_party/"
fi
mkdir -p "$root/dist"
