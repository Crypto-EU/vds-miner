...
static void usage() {
    std::cout <<
R"(vds-miner 1.0.0  —  OpenCL-Miner fuer VDS (Vollar), Equihash(96,5)+Scrypt
...
Geraete:
  ...
  -t, --threads    CPU-Wagner-Threads pro GPU (Standard: 2)
  --cpu-only       Nur CPU, ohne OpenCL
  --intensity      1-24, mehr parallele Nonces (Standard: 8; 6800XT=12, 5700XT=8)
...
Beispiele:
  ...
  vds-miner -o stratum+tcp://vds.666pool.com:9338 -u VcYourAddress -d 0,1 --intensity 12
)";
}
...
static bool self_test() {
    ...
    LOGI("Self-test: Equihash(96,5) Solver (ein Nonce)");
...
static int run_benchmark(OpenClSolver* gpu, int cpu_threads) {
    uint8_t prefix[180] = {};
    const int rounds = 8;
    auto t0 = now_ms();
    ...
    auto worker = [&](int id) {
        for (int r = 0; r < rounds; ++r) {
            ...
            if (gpu && gpu->ready()) gpu->solve(id % gpu->device_count(), prefix, n, on, &cancel);
            else eh_solve_from_header(prefix, n, on, &cancel);
            hashes++;
        }
    };

    int nw = gpu && gpu->ready() ? gpu->device_count() : std::max(1, cpu_threads);
    std::vector<std::thread> th;
    for (int i = 0; i < nw; ++i) th.emplace_back(worker, i);
    for (auto& t : th) t.join();
    auto dt = now_ms() - t0;
    if (dt == 0) dt = 1;
    LOGI("Benchmark: %d Iterationen, %d Loesungen in %llu ms",
...
static void miner_thread(WorkCtx ctx) {
    ...
    while (ctx.stop && !ctx.stop->load() && !g_stop.load()) {
        ...
        while (!g_stop.load() && ctx.stratum->job_epoch() == epoch) {
            ...
            std::atomic<bool> cancel{false};
            if (ctx.gpu && ctx.gpu->ready()) {
                ctx.gpu->solve(ctx.gpu_index, job.header_prefix, nonce, on_sol, &cancel);
            } else {
                eh_solve_from_header(job.header_prefix, nonce, on_sol, &cancel);
            }
            local_h++;
            ctx.stats->hashes++;
...
int main(int argc, char** argv) {
    ...
    bool list_dev = false, bench = false, test = false, cpu_only = false;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (a == "-t" || a == "--threads") cpu_threads = std::stoi(need("-t"));
    ...
    if (list_dev) {
        ...
        if (devs.empty()) {
            LOGI("Keine OpenCL-GPUs. CPU-Mining ist trotzdem moeglich (--cpu-only).");
            return 0;
        }
        for (auto& d : devs) {
    ...
    if (!cpu_only) {
        ...
        if (!gpu.init(devices)) {
            LOGW("OpenCL nicht nutzbar — falle auf CPU zurueck. Fuer AMD 5700XT/6800XT amdgpu-pro oder ROCm OpenCL installieren.");
        }
    }

    if (bench) return run_benchmark(gpu.ready() ? &gpu : nullptr, cpu_threads);
    ...
    LOGI("vds-miner 1.0.0  |  VDS Equihash(96,5)+Scrypt  |  Pool %s:%u  |  User %s",
         host.c_str(), port, user.c_str());

    int nworkers = 0;
    std::vector<std::thread> workers;
    if (gpu.ready()) {
        int per = std::max(1, cpu_threads);
        // 6800XT/5700XT: etwas mehr CPU-Wagner-Threads, Intensity steuert Parallelitaet indirekt
        if (intensity > 0) per = std::max(1, std::min(24, intensity / 2));
        for (int g = 0; g < gpu.device_count(); ++g) {
            auto hint = gpu.devices()[g].board_hint;
            int extra = per;
            if (hint == "6800xt" && intensity < 0) extra = std::max(per, 4);
            if (hint == "5700xt" && intensity < 0) extra = std::max(per, 3);
        ...
        int n = cpu_threads > 0 ? cpu_threads : std::max(1, hw - 1);
        for (int t = 0; t < n; ++t) {
            WorkCtx ctx;
            ctx.stratum = &stratum;
            ctx.gpu = nullptr;
        ...
        LOGI("CPU-Modus mit %d Threads", n);
    }
    LOGI("%d Worker gestartet", nworkers);
